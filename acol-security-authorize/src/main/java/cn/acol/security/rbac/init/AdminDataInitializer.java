package cn.acol.security.rbac.init;
...
public class AdminDataInitializer extends AbstractDataInitializer {
	...
	private void initAdmin(Role role) {
		...
		Set<Role> smid = new HashSet<Role>();
		smid.add(role);
		admin.setRoles(smid);
	...
	protected Resource createResource(String name, String link, String iconName, Resource parent) {
		Resource node = new Resource(); //新建资源
		node.setName(name);
		node.setIcon(iconName);
		node.setParent(parent);
		node.setType(ResourceType.MENU);
		if (StringUtils.isNotBlank(link)) { //资源url 不为空时添加资源实际授权url
		...
		resourceRepository.save(node);//保存资源
		return node;
	}
}
